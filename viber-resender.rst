...
Данный метод позволяет Клиенту не обращаться к API Devino каждый раз, когда требуется получить статус доставки сообщения, а обрабатывать входящие события от платформы Devino на своем внутреннем ресурсе.


При получении статуса сообщения от Viber платформа Devino отправляет HTTP-POST запрос (JSON, UTF-8) на URL сервера Клиента. 
Для каждого из используемых Клиентом адресов отправителя можно настроить отдельный URL для приема статусов сообщений.
В случае, если сервер Клиента возвращает ошибку или не предоставляет ответ, то платформа будет совершать повторные запросы в течение 24 часов.
...
|                 |            | на платформе                               |                                         | Да           |
+-----------------+------------+--------------------------------------------+-----------------------------------------+--------------+
| receivedAt      |DateTime    | Дата и время получения статуса             |                                         |              |
|                 |(YYYY-MM-dd |                                            |                                         | Да           |
|                 |hh:mm:ss)   |                                            |                                         |              |
+-----------------+------------+--------------------------------------------+-----------------------------------------+--------------+
| Status          | String     | Код статуса доставки Viber сообщения.      | **enqueued** – сообщение находится в    |              |
